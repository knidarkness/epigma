     ...
     stages{
         ...
         stage('Deploy'){
             steps{
                script{
                    if (BRANCH_NAME == 'dev'){
                        sh 'ssh ubuntu@52.47.209.237 "cd /usr/epigma/epigma; sudo docker-compose down; cd /usr/epigma/; sudo rm -rf /usr/epigma/epigma; sudo git clone https://github.com/knidarkness/epigma.git; cd /usr/epigma/epigma; sudo docker-compose up --build -d"'
                        sh 'echo "done dev"'
