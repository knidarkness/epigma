     ...
     stages{
         ...
         stage('Deploy'){
             steps{
                script{
                    if (BRANCH_NAME == 'dev'){
                        sh 'ssh ubuntu@52.47.209.237 "sudo mkdir -p /usr/epigma/epigma; cd /usr/epigma/epigma; sudo docker-compose down; cd /usr/epigma/; sudo rm -rf /usr/epigma/epigma; sudo git clone https://github.com/knidarkness/epigma.git; git checkout dev; cd /usr/epigma/epigma; sudo docker-compose up --build -d"'
                        sh 'echo "done dev"'
