...
const Header = ({zoom, zoomTo, undo, redo, enableViewMode, enableDrawMode, mode}) => {
    return (
        <header className="toolbar">
            ...
            <section className="toolbar__items">
                ...
                <button className={`toolbar__button toolbar__button_edit ${(mode === EDITOR_MODE.DRAW) ? 'toolbar__button_active' : ''}`} onClick={() => enableDrawMode()}/>               <button className="toolbar__button toolbar__button_zoom-in" onClick={() => zoomTo(getCenterPoint('#canvas'), Math.max(zoom + 0.05, 0.5))}/>
